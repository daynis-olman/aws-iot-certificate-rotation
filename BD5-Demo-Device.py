...
# - Overview -
# This uses the AWS IoT Jobs Service to receive and execute operations
...
parser.add_argument('--root-ca', help="File path to root certificate authority, in PEM format. " +
                                      ...
                                      "your trust store")
parser.add_argument('--client-id', default="LabDevice", help="Client ID for MQTT connection.")
parser.add_argument('--thing-name', default="LabDevice", help="The name assigned to your IoT Thing")
...
def job_thread_fn(job_id, job_document):
    try:
        ...
        statusDetailsDict = None

        for documents in job_document:
            if documents["packageName"] == "DEVICE_CERTIFICATE_EXPIRING_CHECK":
                print("job_id: "+job_id)
                print(documents)
                statusDetailsDict = { "packageName": "DEVICE_CERTIFICATE_EXPIRING_CHECK" }
                statusDetailsDict["JobId"] = job_id
                statusDetailsDict["ThingName"] = thing_name
                statusDetailsDict = getCSR(statusDetailsDict)
                mqtt_connection.publish(
                    topic="req/"+ thing_name +"/jobs/rotateCert",
                    payload=json.dumps(statusDetailsDict),
...
def on_message_received(topic, payload, **kwargs):
    global newCertificateArn
    print("Received message from topic '{}': {}".format(topic, payload))
    
    myJobPayload = json.loads(payload)
    rotateCert(myJobPayload["certificatePem"])
    newCertificateArn = myJobPayload["certificateArn"]
    print("Calling revokeCert")
    revokeCert()
...
def on_publish_update_job_execution(future):
    ...
    try:
        future.result() # raises exception if publish failed
        print("Published request to update job.", future)
...
def on_update_job_execution_accepted(response):
    ...
    try:
        print("Request to update job was accepted.", response)    
        done_working_on_job()
...
def getCSR (aDict):
    myCSRFile = open('deviceCert-expiring.csr', 'r')  #todo: make this a parameter
    myCSR = myCSRFile.read()
    aDict['csr'] = myCSR
    return aDict

def rotateCert(aCertPem):
    global certificatePath
    print("Rotating the certificate")
    #move current cert to backup
    if os.path.isfile(certificatePath + '.old'):
        os.remove(certificatePath + '.old')
    os.rename(certificatePath, certificatePath + '.old')

    #overwrite current cert
    myCertFile = open(certificatePath, 'w')
    myCertFile.write(aCertPem)
    myCertFile.close()
    return True

def rollbackCert():
    global certificatePath
    print("Rolling back the certificate")
    #restore the backup/original cert
    os.remove(certificatePath)
    os.rename(certificatePath + '.old', certificatePath)
    return True    

def revokeCert():
    global mqtt_connection, jobs_client, thing_name, newCertificateArn

    try:
        print("In revoke Cert")
        #revoke and delete the old cert from IoT Core
        job_id = "Revoke-Cert"
        statusDetailsDict = { "packageName": "CertRotationPackage" }
        statusDetailsDict["JobId"] = job_id
        statusDetailsDict["ThingName"] = thing_name
        statusDetailsDict["certificateArn"] = newCertificateArn  #revoke any certificate for this device EXCEPT for this new one
        print('publishing on {}: {}'.format("req/"+ thing_name +"/jobs/revokeCert", json.dumps(statusDetailsDict)))

        mqtt_connection.publish(
            topic="req/"+ thing_name +"/jobs/revokeCert",
            payload=json.dumps(statusDetailsDict),
            qos=mqtt.QoS.AT_LEAST_ONCE)           

        print("Revokecert Successful")
        
    except Exception as e:
        print("Revokecert Unsuccessful")
        print(e)
        rollbackCert()
...
if __name__ == '__main__':
    ...
    print("Certificate path is", certificatePath)
    ...
    try:
        ...
        reply_topic = 'res/{}/jobs/'.format(args.client_id)
        print("Subscribing to topic '{}'...".format(reply_topic))
